...
#include "fib.h"
#include "fibpriv.h"
//#include "mex.h"  // [CPS] need mxIsNaN
...
double* D = NULL;
double* S = NULL;
double* W = NULL;  // Local cost = 1/speed
double* Q = NULL;
	...
	while( !fh_isempty(open_heap) && num_iter<nb_iter_max && !stop_iteration )
	{
		...
		*/
		...
		for( int k=0; k<4; ++k )
		{
			...
			{
				double P = h/W_(ii,jj);    //  (1/width)/(cost) = speed/width
				...
				else if( ((int) S_(ii,jj)) == kFar )
				{
					...
					if( L==NULL ) // [CPS] added check for NaN in constraint map and cut:  && A1<=L_(ii,jj)
					{
						S_(ii,jj) = kOpen;
