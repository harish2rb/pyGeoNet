...
def main():
    ...
    geodesicPathsCellDic, numberOfEndPoints = pyg_ntd.Channel_Definition(xx, yy,
                                                                         geodesicDistanceArray,
                                                                         basinIndexArray,
                                                                         flowDirectionsArray)
...
if __name__ == '__main__':
    t0 = clock()
    defaults.doBatchRun = 1
    tif_file_names = glob.glob(parameters.demDataFilePath + "/*.tif")
    for tif_file_name in tif_file_names:
        print(tif_file_name.split('/')[-1])
        parameters.demFileName = tif_file_name.split('/')[-1]

        new_geonetResultsDir = os.path.join(parameters.geoNetHomeDir, "results",
                                            parameters.demFileName[:-4])
        new_geonetResultsBasinDir = os.path.join(parameters.geoNetHomeDir, "basinTiffs")

        # Write shapefile file paths
        new_shapefilepath = os.path.join(parameters.geoNetHomeDir, "results",
                                         parameters.demFileName[:-4])
        driverName = "ESRI Shapefile"

        new_pointshapefileName = parameters.demFileName[:-4] + "_channelHeads"
        new_pointFileName = os.path.join(new_shapefilepath, new_pointshapefileName + ".shp")

        new_drainagelinefileName = parameters.demFileName[:-4] + "_channelNetwork"
        new_drainagelineFileName = os.path.join(new_shapefilepath,
                                                new_drainagelinefileName + ".shp")

        new_junctionshapefileName = parameters.demFileName[:-4] + "_channelJunctions"
        new_junctionFileName = os.path.join(new_shapefilepath, new_junctionshapefileName + ".shp")

        new_streamcellFileName = os.path.join(new_geonetResultsDir,
                                              parameters.demFileName[:-4] + "_streamcell.csv")

        new_xsshapefileName = parameters.demFileName[:-4] + "_crossSections"
        new_xsFileName = os.path.join(new_shapefilepath, new_xsshapefileName + ".shp")

        new_banklinefileName = parameters.demFileName[:-4] + "_bankLines"
        new_banklineFileName = os.path.join(new_shapefilepath, new_banklinefileName + ".shp")

        # Things to be changed
        # PM Filtered DEM to be used in GRASS GIS for flow accumulation
        new_pmGrassGISfileName = os.path.join(new_geonetResultsDir, "PM_filtered_grassgis.tif")

        parameters.pmGrassGISfileName = new_pmGrassGISfileName
        parameters.geonetResultsDir = new_geonetResultsDir
        parameters.geonetResultsBasinDir = new_geonetResultsBasinDir
        parameters.shapefilepath = new_shapefilepath
        parameters.pointFileName = new_pointFileName
        parameters.pointshapefileName = new_pointshapefileName
        parameters.drainagelinefileName = new_drainagelinefileName
        parameters.drainagelineFileName = new_drainagelineFileName
        parameters.junctionshapefileName = new_junctionshapefileName
        parameters.junctionFileName = new_junctionFileName
        parameters.streamcellFileName = new_streamcellFileName
        parameters.xsshapefileName = new_xsshapefileName
        parameters.xsFileName = new_xsFileName
        parameters.banklinefileName = new_banklinefileName
        parameters.banklineFileName = new_banklineFileName

        if not os.path.exists(new_geonetResultsDir):
            os.mkdir(new_geonetResultsDir)
